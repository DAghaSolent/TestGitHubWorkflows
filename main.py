...
app.layout = html.Div([
    dbc.Row(html.Div(
        """This is a demo of how we can visalise very large time series data sets using a column orientated database. """)),
    dbc.Row(html.Strong(
        """Drag the X axes to increase/decrease/pan the date ranges. Note the resulting query durations.""")),
    dbc.Row(html.Div(
        """The top chart plots a raw time series value, ie with no downsampling (with a result count limit set in [A]). """)),
    dbc.Row(html.Div("""The lower chart is identical except that the time series values are downsampled by the database to the number entered in input [B].
             """)),
    dbc.Row(html.Div([html.Span(
        """This is a live demo. The release tech stack is docker container running a Flask Plotly Dash app. The plots are Plotly Express Line plots. """),
                      html.Span("""There are two more containers running ClickhouseDB. """), html.Br(),
                      html.Span("""The downsampling query uses the Clickhouse """),
                      dcc.Link('largestTriangleThreeBuckets',
                               href="https://clickhouse.com/docs/en/sql-reference/aggregate-functions/reference/largestTriangleThreeBuckets",
                               target='_blank'),
...
def update_output(std_window, max_nonds, agg_relayout, relayout):
    ...
    try:
        ...
        fig.update_layout(title_text=f"Raw Chart of {fig_row_count} samples, limited to {raw_limit} {elapsed} ",
                          title_x=0.5, xaxis_title='Date and Time', yaxis_title='Raw Value')
        fig.update_xaxes(range=[start_date_str,
                                to_date_str])  # Don't let chart autoscale sa loses impact of how few samples we're pulling compared to downsampled
        ...
        fig_agg.update_layout(
            title_text=f"Downsampled Chart ({fig_agg_row_count}/{std_window} of {res_count:,} rows) query time:{elapsed:0.2f}s",
            title_x=0.5,
